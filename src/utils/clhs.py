# Edited by Madeline Pernat, CU Boulder, Nov. 2023, to perform "Uniform Conditioned Latin Hypercube Sampling". Instead
# of stratifying quantiles, stratify over each feature's value range. Two parts modified: 1) get_strata function and
# 2) categorical predictor proportion calculation in the clhs function
...
def get_strata(predictors, num_samples, good_mask=None):
    ...
    # Define edges of the strata
    mins = np.min(np.squeeze(predictors)[good_mask], axis=0)
    maxs = np.max(np.squeeze(predictors)[good_mask], axis=0)
    strata = np.zeros(shape=(num_samples+1, len(predictors.columns)))
    for i in range(0, len(predictors.columns)):
        start = mins[i]
        stop = maxs[i]
        edges = np.linspace(start, stop, num = num_samples + 1)
        strata[:, i] = edges
...
def clhs(predictors, num_samples, good_mask=None, include=None,
    ...
    **progress_kwargs
    ...
    if n_categorical > 0:
        # Edited by MRP - equal proportions
        categorical_proportions = dict()
        for column in categorical_predictors.columns:
            vals = np.unique(categorical_predictors[column])
            n_vals = len(vals)
            equal_proportion = 1 / n_vals
            proportions = pd.Series([equal_proportion] * n_vals).set_axis(vals)
            categorical_proportions[column] = proportions
